...
class CycleEDF_new():
    ...
    def __init__(self,
        ...
        self.wc_exec_time=np.array(wc_exec_time)
    ...
    def compute(self) -> np.ndarray:
        ...
        #setting up period counter to keep track of deadlines 
        period_counter=np.ones(self.periods.shape,dtype=int)
        inv_counter=np.zeros(self.periods.shape,dtype=int)
        ...
        while any(inv_counter<self.num_invocations):
            ...
            if running_task is None:
                ...
                if inv_counter[nearest_task]<self.num_invocations:
                    #extracting invocation time
                    invocation_row=period_counter[nearest_task]
                ...
                if all(task_end_time<next_deadlines):
                    ...
                    if inv_counter[interrupting_task]<self.num_invocations:
                        #extracting invocation time
                        invocation_row=period_counter[interrupting_task]-1
